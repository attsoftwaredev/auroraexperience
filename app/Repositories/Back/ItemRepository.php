...
class ItemRepository
{
    ...
    public function store($request)
    {
        ...
        if ($file = $request->file('photo')) {
            $images_name = ImageHelper::ItemhandleUploadedImage($request->file('photo'),'public/assets/images');
    ...
    public function stores($request)
    {
        ...
        // subscription product file upload
    ...
    public function update($item,$request)
    {
        ...
        if ( $request->file('photo')) {
            ...
            $images_name = ImageHelper::ItemhandleUpdatedUploadedImage($request->photo,'/public/assets/images',$item,'/public/assets/images/','photo');
            $input['photo'] = $images_name[0];
            $input['thumbnail'] = $images_name[1];
        }
            ...
            if(!$request->hasFile('file')){
                if($request->link){
                    if(file_exists('public/assets/files/'.$item->file)){
                        unlink('public/assets/files/'.$item->file);
                    }
                    $input['file'] = null;
                }
            }
    ...
    public function delete($item)
    {
        ...
        ImageHelper::handleDeletedImage($item,'photo','public/assets/images/');
        ImageHelper::handleDeletedImage($item,'thumbnail','public/assets/images/');
        if($item->item_type == 'digital' && $item->file){
            ImageHelper::handleDeletedImage($item,'file','public/assets/files/');
        }
        $item->delete();
    }
    ...
    {
        ImageHelper::handleDeletedImage($gallery,'photo','/public/assets/images/');
        $gallery->delete();
    }
    ...
    public function storeImageData($request,$item_id=null)
    {
        ...
        if ($galleries = $request->file('galleries')) {
            foreach($galleries as $key => $gallery){
                $storeData[$key] = [
                    'photo'=>  ImageHelper::handleUploadedImage($gallery,'public/assets/images'),
