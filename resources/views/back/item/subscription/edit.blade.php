...
<div class="mb-4">
        <div class="d-sm-flex align-items-center justify-content-between">
            <h3 class="mb-0 bc-title dash-title"><b>{{ __('Update Subscription Product') }}</b> </h3>
        ...
        <div class="col-lg-8">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <img class="admin-img lg" src="{{ $item->photo ? asset('public/assets/images/'.$item->photo) : asset('public/assets/images/placeholder.png') }}" >
                    </div>
                            ...
                            <div class="d-block gallery_image_view">
                                ...
                                @forelse($item->galleries as $gallery)
                                    <div class="single-g-item d-inline-block m-2">
                                            ...
                                            <a class="popup-link" href="{{ $gallery->photo ? asset('public/assets/images/'.$gallery->photo) : asset('public/assets/images/placeholder.png') }}">
                                                <img class="admin-gallery-img" src="{{ $gallery->photo ? asset('public/assets/images/'.$gallery->photo) : asset('public/assets/images/placeholder.png') }}"
                                                    alt="No Image Found">
                                            </a>
                                    </div>
                                @empty
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group view_file {{$item->file_type == 'file' ? '' : 'd-none'}} ">
                        ...
                        @if (file_exists('public/assets/files/'.$item->file))
                            <a download href="{{asset('public/assets/files/'.$item->file)}}">{{__('Download File')}}</a>
                        @endif
        ...
        <div class="col-lg-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <button type="submit" class="btn btn-secondary mr-2 saveBtn">{{ __('Save') }}</button>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="subcategory_id" id="subcategory_id" class="form-control schild_subscription_category">
                            <option value="" selected disabled>{{__('Select One')}}</option>
                            @foreach(DB::table('ssubcategories')->whereStatus(1)->get() as $cats)
							<option value="{{ $cats->id }}"  data-id="{{ $cats->category_id }}"  class="child_cat category_child_child{{ $cats->category_id }}  hide" {{ $cats->id == $item->subcategory_id ? 'selected' : '' }}>{{ $cats->name }}</option>
							@endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <select name="childcategory_id" id="childcategory_id" class="form-control">
                            <option value="" selected disabled>{{__('Select One')}}</option>
                            @foreach(DB::table('schild_categories')->whereStatus(1)->get() as $catss)
							<option value="{{ $catss->id }}"  data-id="{{ $catss->subcategory_id }}"  class="schild_cat category_child_schild{{ $catss->subcategory_id }}  hide" {{ $catss->id == $item->childcategory_id ? 'selected' : '' }}>{{ $catss->name }}</option>
							@endforeach
                        </select>
                    </div>
                    <!------
...
@section('scripts')
    ...
    <script>
    $('.schild_subscription_category').on('change', function () {

        $('.schild_cat').removeClass('display');
        $('.schild_cat').addClass('hide');
        
        var subid = $('.category_child_schild'+ this.value).data('id'); 
        
        var id = $('.schild_subscription_category').val();
        
        $('.category_child_schild' + this.value).each(function() {
