...
<tr id="product-bulk-delete">
  <td><input type="checkbox" class="bulk-item" value="{{$data->id}}"></td>
    <td>
        <img src="{{ $data->thumbnail ? asset('public/assets/images/'.$data->thumbnail) : asset('public/assets/images/placeholder.png') }}" alt="Image Not Found">
    </td>
    <td>
        {{ $data->name }}
    </td>
    ...
    <td>
        <div class="dropdown">
            ...
            <div class="dropdown-menu animated--fade-in" aria-labelledby="dropdownMenuButton">
              ...
              @elseif($data->item_type =='subscription')
              <a class="dropdown-item" href="{{ route('back.subscription.item.edit',$data->id) }}"><i class="fas fa-angle-double-right"></i> {{ __('Edit') }}</a>
